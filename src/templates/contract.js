...
const Contract = ({data}) => {
  const getBaseContract = baseContract =>
    data.allSource.nodes
      ...
      .reduce(
        (contracts, source) => contracts.concat(source.childrenContract),
        []
      )
      .filter(contract => contract.name === baseContract.name)[0]
      ...
      {data.contract.childrenBaseContract.length > 0 && (
        <h4 className="subtitle is-5">
          {data.contract.childrenBaseContract.map((baseContract, index) => (
            <>
              {getBaseContract(baseContract) ? (
                <Link to={`/${getBaseContract(baseContract).id}`}>
                  {baseContract.name}
                </Link>
              ) : (
                <span>{baseContract.name}</span>
              )}
